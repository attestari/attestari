...
- **`NotariRetriever`** (`BaseRetriever`) — recall a subject's relevant facts as
...
## Use — inject memory into a chain (NotariRetriever)
...
retriever = NotariRetriever(
    mem=mem,                         # the Notari engine
...
> `notari[anthropic]` and set `ANTHROPIC_API_KEY` for Claude-powered extraction.
...
## Use — conversational memory (NotariChatMessageHistory)

```python
from notari import Memory
from notari_langchain import NotariChatMessageHistory
from langchain_core.messages import HumanMessage

history = NotariChatMessageHistory(mem=Memory(), subject_id="user_42")
...
chain_with_memory = RunnableWithMessageHistory(
    chain,
    lambda session_id: NotariChatMessageHistory(mem=Memory.local(), subject_id=session_id),
...
|---|---|---|
| `mem` | — | The Notari engine: `Memory()`, `Memory.local()`, or `Memory.postgres()`. |
